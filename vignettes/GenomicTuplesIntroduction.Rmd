...
`r Biocpkg("GenomicRanges")` package (and can be accessed by typing 
`vignette("GenomicRangesIntroduction", package = "GenomicRanges")` at the R 
console). While not essential, familiarity with the 
`r Biocpkg("GenomicRanges")` will be of benifit in understanding the 
`r Biocpkg("GenomicTuples")` package.
...
__Seqinfo__ can be extracted using the `seqinfo` accessor:
...
```{rsize-method, eval = TRUE, echo = TRUE}
size(gt3)
```
...
The `elementLengths` method returns a list of integers
...
Genomic tuples are __always__ treated as genomic ranges when searching for  
...
```{r3-tuples-compare-examples, eval = TRUE, echo = TRUE}
# Construct example 3-tuples
...
`r Biocpkg("GenomicTuples")` uses heavily and is based upon. A special thanks to Hervé Pagès for his assistance and fixes.
